...
`/api/match/status`、心跳响应、SSE 增加 `phase`（waiting_players / active / reconnecting / finished）、`serverTime` 和 `participants`。每个参与者只暴露 ID、阵营、是否本人、状态（joining / online / disconnected）与截止时间，不包含令牌、连接标识或密码字段。

SSE 每秒检查状态变化并立即推送，未变化时每8秒发心跳。客户端按服务器时间计算倒计时，并显示对手离线、重连、入场和结果。游戏 WebSocket 断开后保留生命周期订阅，以便在宽限期内接收结算；不再因关闭订阅而丢失最终结果。

新增战局有2分钟入场期限，避免无人进入的房间永久占用槽位；断线后的60秒宽限不变。正常结算、投降和超时结束的内核约20秒后回收。重启造成内核缺失仍按中断处理，不给玩家算正常失败。

网页还独立监测游戏状态：6秒无状态，或状态仍到达但游戏时10秒不变，显示常驻警告并暂停操作；收到有效推进后解除。不能用 ping 正常推断模拟正常，也不能仅凭客户端断线断言服务器进程崩溃。

持续兵线修复位于共享JS内核，不是定时重发命令的插件补丁：`orderOwner` / `plannedOrderOwners` 标记玩家命令，使其不被AI临时路线的生产周期清理规则删除。单机、模拟与服务器必须同时使用这一份内核；Go宿主不需要改写。

截至本次部署配置，线上 maxKernels 为4，插件PvP每局2人（配置容量8人）；宿主约1核/1GB内存。该数值不是四局高倍率满载流畅的压测保证，结束回收中的内核也短暂占用槽位。
